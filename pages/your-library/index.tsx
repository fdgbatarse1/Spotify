...
const YourLibrary = () => {
  ...
  const favoriteAlbums = useAppSelector(
    (state) => state.favorites.favoriteAlbums
  );
  ...
  useEffect(() => {
    if (favoriteAlbums?.items?.length && favoriteTracks?.items?.length) {
      if (
        favoriteAlbums?.items?.length > 0 ||
        favoriteTracks?.items?.length > 0
      ) {
