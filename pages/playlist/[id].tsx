...
const Playlist = () => {
  ...
  const playlistError = useAppSelector(
    (state) => state.playlists.playlistError
  );
  const playlistStatus = useAppSelector(
    (state) => state.playlists.playlistStatus
  );
  ...
  const unsuccess = useCheckStatus(
    playlistStatus,
    playlistError,
    <Header goBack={true} />
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-6">
        <PlaylistDescription />
        <TracksList
          tracks={convertPlaylistTracks(playlistTracks)}
